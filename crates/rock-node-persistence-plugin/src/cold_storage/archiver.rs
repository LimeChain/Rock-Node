...
impl Archiver {
    ...
    pub fn run_archival_cycle(&self) -> Result<()> {
        ...
        if current_block_count < self.config.hot_storage_block_count + self.config.archive_batch_size {
            return Ok(());
        }

        info!("Hot tier count ({}) exceeds trigger, starting archival...", current_block_count);

        let num_to_archive = self.config.archive_batch_size;
        let blocks_to_archive = self.hot_tier.read_block_batch(earliest_hot_u64, num_to_archive)?;
        info!("Read {} blocks from hot tier for archival.", blocks_to_archive.len());

        let new_index_path = self.cold_writer.write_archive(&blocks_to_archive)?;
        info!("Successfully wrote blocks to cold archive: {:?}", new_index_path.file_name().unwrap_or_default());
        
        if let Err(e) = self.cold_reader.load_index_file(&new_index_path) {
             // If this fails, we have an inconsistent state. This is a critical error.
        ...
        for i in 0..num_to_archive {
            self.hot_tier.add_delete_to_batch(earliest_hot_u64 + i, &mut batch)?;
        }
